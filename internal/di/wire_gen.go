...
func InitializeInternalApplication(context2 context.Context) (*InternalApplicationAPI, func(), error) {
	serverCustomizer := server.ProvideInternalCustomizer()
	serverServer, cleanup, err := server2.ProvideServer(serverCustomizer)
	if err != nil {
		return nil, nil, err
	}
	appProperties := config.ProvideAppProperties()
	internalAPIConfiguration := config.ProvideInternalAPIConfiguration(appProperties)
	internalApplicationAPI := &InternalApplicationAPI{
		Server: serverServer,
		Config: internalAPIConfiguration,
	}
	return internalApplicationAPI, func() {
		cleanup()
	}, nil
}
